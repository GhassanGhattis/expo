...
class RuntimeDecorator : public Base, private jsi::Instrumentation {
 ...
 protected:
  ...
  size_t byteLength(const TypedArrayBase &tab) override {
    return plain_.byteLength(tab);
  }
  uint8_t* data(const ArrayBuffer& ab) override {
    return plain_.data(ab);
  };
  ArrayBuffer data(const TypedArrayBase& tab) override {
    return plain_.data(tab);
  }
